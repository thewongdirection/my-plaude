...
| **Diarization** | `--diarize` via **pyannote** (or **whisperx**, Python only). Version-aware model pick (pyannote 4.x → `speaker-diarization-community-1`, 3.x → `speaker-diarization-3.1`); `--diarize-model`, `--num/min/max-speakers`. **The PowerShell port runs pyannote directly** (bundled `pyannote_diarize.py` helper) — same engine, same speaker labels, same flags honored — not whisper-ctranslate2's built-in diarization. |
...
- **Diarization needs a specific stack on Windows** (both tools): a **Python 3.11 venv** with `pyannote.audio 3.1.1` + `torch/torchaudio 2.2.2` + `numpy 1.26.4` + `huggingface_hub 0.23.4` + `speechbrain 0.5.16` (avoids `k2`, which has no Windows wheels). Python 3.13 works for transcription/translation but not pyannote. See the [README tech-stack table](README.md#tech-stack-that-works-for-both-versions) for the exact verified combination; WSL2/Linux installs pyannote 4.x + k2 cleanly.
